...
## Introduction
This document demonstrates basic data analysis and plotting using the HAVEN dataset in R. In doing so we will plot and work with data for age, sex, height, weight, bmi, lesion volume, number of lesions, and education level. We will firstly plot this data using a variety of plots, to understand how certain graphs are preferred for certain data. After plotting some of these, we will perform basic statistical tests to understand whether the relationship between the variables are significant.

# Load and display the data
First thing first, we need to load and read the data. It's in a format called 'comma separated values' or CSV, which we can read into R using the `read_csv` function. We assign the data to a variable called `data`, which we can now work directly with. You can see this on the right hand panel in R ->

```{r include = FALSE}
data <- read_csv("../data/participant_data.csv")
```

If we want to find out some basic information about the data, we can display the first few rows of the data using the `head` function:
```{r}
head(data)
```

and calculate summary statistics for each variable. The summary statistics will give us the mean, range etc for each.
```{r}
summary(data)
```
...
So, you can see that (generally) the older you are, the higher the total lesion volume is.

### Violin plot of Total lesion volume by sex
We can also group these results by sex, if we wanted to know whether there was a gender effect. We can do this using a boxplot, but because we are a but fancy, we can use a violin plot, a type of boxplot but which looks a bit nicer.
```{r}
...
If we wanted to understand whether there is a difference between the height and weight between Males and Females, we can do this using violin plot, but also by creating a scatter plot:
```{r}
...
Finally, we may be interested with understanding the potential impact of demographic and brain factors upon behaviour, we can look at participants' reaction times and accuracy during the episodic memory task. 
...
But you can see that there is an outlier for the last two graphs. What would happen if we removed this from the plot?
...
# Subset the data to exclude lesion_volume greater than 5
rt_data_subset <- subset(rt_data, lesion_volume <= 5)
...
print(plot_lv_rt)
```
So you can see a difference, although none of the relationships become significant.
